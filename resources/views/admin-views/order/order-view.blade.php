...
{{--            </div>--}}
  ...
  <div class="form--control justify-content-end ">
    <form action="{{ route('admin.orders.editOrderItems', ['order_id' => $order['id']]) }}" method="get">
  {{--    @csrf--}}
      <input type="hidden" name="order_id" id="order_id" value="{{ $order['id'] }}" />

      <button type="submit" class="btn btn--info print--btn" style="min-width: 30px;">
        <i class="tio-edit"></i>
      </button>
                                ...
                                <a class="btn btn--info print--btn" target="_blank" href={{route('admin.orders.generate-invoice',[$order['id']])}}>
                                    <i class="tio-print mr-1"></i> a6 {{translate('print')}} {{translate('invoice')}}
                                </a>
                                <a class="btn btn--info print--btn" target="_blank" href={{route('admin.orders.generate-invoicea4',[$order['id']])}}>
                                    <i class="tio-print mr-1"></i> A4 {{translate('print')}} {{translate('invoice')}}
                                </a>
...
{{--                                    <th class="border-0">{{translate('product_id')}}</th>--}}
                                    <th class="text-center border-0">{{translate('Item details')}}</th>
                            ...
                            @foreach($order->details as $detail)
                                ...
                                <tr>
                                      ...
                                      <!--  <td>
{{--                                          <h5 class="line--limit-1">{{$product['id']}}</h5>--}}
...
<!-- <a class="action-btn" href="{{ route('admin.orders.edit', ['product_id' => $product['id']]) }}">
        <i class="tio-edit"></i>
</a> -->



{{--<form action="{{ route('admin.orders.edit', ['product_id' => $product['id']]) }}" method="get">--}}
{{--    <input type="hidden" name="order_id" id="order_id" value="{{ $order['id'] }}" />--}}

{{--    <button type="submit" class="btn btn-outline-info" style="min-width: 30px;">--}}
{{--    <i class="tio-edit"></i>--}}
{{--    </button>--}}
{{--</form>--}}
...
{{--        <a type="submit" class="btn btn-outline-danger" onclick="return confirm('Are you sure you want to delete this product?')">--}}
{{--          <i class="tio-delete-outlined"></i>--}}
{{--        </a>--}}
{{--      </form>--}}
    ...
    <form action="{{ route('admin.orders.delete', ['product_id' => $product['id']]) }}" method="get">
        ...
        <button style="min-width: 30px;" type="submit" class="btn btn-outline-danger" onclick="return confirm('Are you sure you want to delete this product?')">
            <i class="tio-delete-outlined"></i>
        </button>
    </form>
...
-->
    ...
    <!-- End Dropdown -->
                            ...
                            <div class="d-flex flex-wrap g-2">
                                <div class="hs-unfold w-0 flex-grow min-w-160px">
                                    <label class="input-date">
                                        <input class="js-flatpickr form-control flatpickr-custom min-h-45px form-control" type="text" value="{{ date('d M Y',strtotime($order['delivery_date'])) }}" name="deliveryDate" id="from_date" data-id="{{ $order['id'] }}"  required>
                                    </label>
                                </div>
                                <div class="hs-unfold w-0 flex-grow min-w-160px">
                                  <label>
     ...
     src="{{asset('storage/app/public/branch/'.$order->branch->image)}}" alt="Image Description">
                        </div>
                        <div class="media-body">
                            <span class="fz--14px text--title font-semibold text-hover-primary d-block">
                                {{$order->branch->name}}

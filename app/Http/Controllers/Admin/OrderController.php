		...
		if ($request->order_status == 'returned' || $request->order_status == 'failed' || $request->order_status == 'canceled') {
			...
			foreach ($order->details as $detail) {
				if ($detail['is_stock_decreased'] == 0) {
					...
					if ($product != null) {
						//check stock
						// commented this code 11/12/2023
...
//    dd($footer_text);
  ...
  public function generate_invoicea4($id): \Illuminate\Contracts\View\Factory  | \Illuminate\Contracts\View\View  | \Illuminate\Contracts\Foundation\Application
	{
    $order = $this->order->where('id', $id)->first();
    $footer_text = $this->business_setting->where(['key' => 'footer_text'])->first();
//    dd($order);

		return view('admin-views.order.invoicea4', compact('order', 'footer_text'));
	}
...
//  new stuffs 26-07-2023
	public function delete(Request $request): RedirectResponse{
		...
		// $order_detail = $this->order_detail->where(['product_id' => $detail['id']]);
		// $order = $this->order->with('details')->where(['id' => $id])->first();
...
//		dd($order->details);
	...
	public function deleteProduct(Request $request, $product_id) {
		// $order_detail = $this->order_detail::find($product_id);
        $order_id = $request->input('order_id');
		$orderDetail = $this->order_detail::where('product_id', $product_id)->where('order_id', $order_id)->first();
		// dd($orderDetail);
        if ($orderDetail) {
            $orderDetail->delete();
            // The record with the matching product_id and order_id has been deleted.
            // You may perform additional actions after deletion if needed.
        } else {
            // Handle the case when no matching record was found.
        }


		// Redirect back to the order details page or any other appropriate page
...
//    dd($orderDetail);
    return view('admin-views.order-detail.edit', compact('order_id','orderDetail','product_details','categories', 'products', 'category', 'keyword', 'branches', 'users'));
		// return redirect()->back()->with('success', 'Product edited successfully!');
	}

  //editProduct / editOrderItems
  public function editOrderItems(Request $request,$order_id)
  {
    $category = $request->query('category_id', 0);
    $categories = $this->category->active()->get();
    $keyword = $request->keyword;
...
//    dd($request);
//        dd($products);
    return view('admin-views.order-detail.add_product_to_order', compact('categories', 'products', 'category', 'keyword', 'branches', 'users'));


  }
    public function updateOrderDetails(Request $request, $order_id, $product_id, $id)
  {
  //  dd($request);
    ...
    if (!$orderDetail) {
      // Handle the case where the order details are not found (optional)
    ...
    $orderDetail->variation = $request->input('variation');
    $orderDetail->product_details = $request->input('product_details');
  ...
  // Update other fields as needed
    ...
    // Optionally, you can redirect the user back to the order view page with a success message
    return redirect()->route('orders.show', ['id' => $order_id])->with('success', 'Order details updated successfully');
  }

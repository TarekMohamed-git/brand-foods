...
//        $order->id = $order_id;
      ...
      // Check if an order_id is provided in the request
      $order_id = $request->edit_id_order;
      $existing_order = null;
//      dd($request->edit_id_order);
      // If order_id is provided, retrieve the existing order
      if ($order_id) {
        $existing_order = $this->order->find($order_id);
      }

      // If an existing order is not found, create a new order
      if (!$existing_order) {
        $order_id = 100000 + $this->order->all()->count() + 1;
        if ($this->order->find($order_id)) {
          $order_id = $this->order->orderBy('id', 'DESC')->first()->id + 1;
...
//        dd($existing_order);
        ...
        // Update the existing order attributes as needed
        $order = $existing_order;
        // You may add other attributes here if needed, e.g., update the order status, delivery address, etc.
        // For example:
        // $order->delivery_address_id = $request->delivery_address_id ?? $order->delivery_address_id;
...
//      dd($existing_order);
    ...
    public function generate_invoicea4($id): \Illuminate\Http\JsonResponse
    {
        $order = $this->order->where('id', $id)->first();
        ...
        return response()->json([
            'success' => 1,
            'view' => view('admin-views.pos.order.invoice_a4', compact('order'))->render(),
        ]);
    }

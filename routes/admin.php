...
Route::group(['namespace' => 'Admin', 'as' => 'admin.'], function () {
	...
	Route::group(['middleware' => ['admin', 'employee_active_check']], function () {
		...
		Route::group(['prefix' => 'pos', 'as' => 'pos.', 'middleware' => ['module:pos_management']], function () {
			...
			Route::get('invoicea4/{id}', 'POSController@generate_invoicea4'); //a4_print
...
//orders
			...
			Route::get('generate-invoicea4/{id}', 'OrderController@generate_invoicea4')->name('generate-invoicea4')->withoutMiddleware(['module:order_management']);
      ...
      Route::get('edit-order-items/{order_id}', 'OrderController@editOrderItems')->name('editOrderItems'); // edit one item from order

      Route::get('delete/{product_id}', 'OrderController@deleteProduct')->name('delete'); // delete one item from order
			Route::get('edit/{product_id}', 'OrderController@editProduct')->name('edit'); // edit one item from order
			Route::post('update/{product_id}', 'OrderController@updateProduct')->name('update'); // edit one item from order
      Route::post('update-order-details/{order_id}/{product_id}/{id}', 'OrderController@updateOrderDetails')->name('update.order.details');
		...
		Route::group(['prefix' => 'order', 'as' => 'order.', 'middleware' => ['module:order_management']], function () {
			...
			Route::delete('delete/{id}', 'OrderController@delete')->name('delete'); // delete one order with items
		});
